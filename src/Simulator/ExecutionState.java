...
/**
 * Created by johnchronis on 4/11/17.
 */
public class ExecutionState {
    ...
    public boolean canSchedule(Long nextOpid, Long contID,long clock) {
        boolean result = true;
        if(plan.opIdtoStartEndProcessing_MS.get(nextOpid).a > clock){ return false;}
        if(workerAvailableAt.get(contID) > clock){return false;}
        if(!isWorkerAvailable.get(contID)){return false;}
